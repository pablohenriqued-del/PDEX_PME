...
18. **Mobile drawer + swipe gesture (Feb 2026)**: `Sheet` shadcn abrindo pela esquerda em telas <1024px, com todos os itens da sidebar. Botão hamburger + swipe da borda esquerda (24px, delta 60px, dominante horizontal).
19. **Bottom Nav Mobile (Feb 2026)**: `MobileBottomNav` fixo em `lg:hidden` com 5 slots — Dashboard, CRM, Pedidos, Sino (badge unread com pulse), Menu (abre drawer). Safe-area-inset para iOS. Main content ganha `pb-16 lg:pb-0` para não sobrepor.
20. **Apresentação executiva HTML (Feb 2026)**: `/apresentacao.html` — 9 slides self-contained (Capa · Problema · Solução · Módulos · Multi-Tenant · Stack · Números · Roadmap · CTA). Animações CSS `reveal` com delays escalonados, brand mark PDEX animado, halo blur, grain overlay, dots interativos, progress bar, keyboard (←/→/espaço/Home/End) + swipe touch. Sem dependências externas exceto Google Fonts.
